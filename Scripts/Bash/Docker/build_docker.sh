...
write_info "build_docker" "Creating and pushing Docker manifest for ${TAG_PREFIX}:${TAG_SUFFIX}-latest"

MANIFEST_TAG="${TAG_PREFIX}:${TAG_SUFFIX}-latest"
docker manifest create "$MANIFEST_TAG" \
$(for build_architecture in "${BUILD_ARCHITECTURES[@]}"; do
    echo "${TAG_PREFIX}:${TAG_SUFFIX}-latest-${build_architecture//\//-}"
done)

# Annotate the manifest with each architecture
for build_architecture in "${BUILD_ARCHITECTURES[@]}"; do
    docker manifest annotate "$MANIFEST_TAG" \
    "${TAG_PREFIX}:${TAG_SUFFIX}-latest-${build_architecture//\//-}" \
    --os linux --arch "${build_architecture%%/*}" \
    $(if [[ "$build_architecture" == *"/"* ]]; then echo "--variant ${build_architecture##*/}"; fi)
done

docker manifest push "$MANIFEST_TAG"

if ! write_response "build_docker" "Manifest created and pushed: \"$MANIFEST_TAG\""; then
    write_error "build_docker" "Failed: Unable to push manifest \"$MANIFEST_TAG\""
    exit 1
fi

write_info "build_docker" "Successfully created and pushed Docker manifest: \"$MANIFEST_TAG\""
