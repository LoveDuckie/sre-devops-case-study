...
if ! is_docker_installed; then
    write_error "build_docker_services" "Failed: Docker is not installed on this system"
    exit 1
fi

if ! is_command_available yq; then
    write_error "build_docker_services" "Failed: Unable to find the command \"yq\""
    exit 1
fi
...
if [ ! ${#BUILD_SERVICES[@]} -gt 0 ]; then
    write_error "build_docker_services" "No container images to build."
    exit 3
fi
...
if ! is_valid_docker_builder "$BUILDER_NAME"; then
    write_info "build_docker_services" "Creating Builder: \"$BUILDER_NAME\""
...
for build_service in ${BUILD_SERVICES[@]}; do
    write_info "build_docker_services" "*********************************"
    write_info "build_docker_services" "Building Service: \"$build_service\""
    write_info "build_docker_services" "*********************************"
    write_info "build_docker_services" "↪ Service: $build_service"
    ...
    write_info "build_docker_services" "$SERVICE_DOCKERFILE_PATH_ABS"
    ...
    write_info "build_docker_services" "↪ Build Version: $BUILD_VERSION"
    SERVICE_DOCKERFILE_PATH=$(yq ".services[\"$build_service\"][\"build\"][\"dockerfile\"]" $DOCKER_PROJECT_COMPOSE_FILEPATH)
    if [ -z "$SERVICE_DOCKERFILE_PATH" ]; then
        write_error "build_docker_services" "The Dockerfile for \"$build_service\" could not be resolved."
        continue
    fi
    ...
    for build_type in ${BUILD_TYPES[@]}; do
        write_info "build_docker_services" "Building type: \"$build_type\""
        ...
        for build_architecture in "${BUILD_ARCHITECTURES[@]}"; do
            ...
            write_info "build_docker_services" "↪ Architecture: \"$build_architecture\""
            write_info "build_docker_services" "↪ Tag (Architecture): \"$TAG_ARCH\""
